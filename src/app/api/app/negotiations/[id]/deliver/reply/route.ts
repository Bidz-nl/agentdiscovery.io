...
import { applyConsumerDeliveryReplyTransition } from '@/lib/adp-v2/negotiation-lifecycle'
...
export async function POST(
  ...
  if (negotiation.initiatorDid !== ownerSession.activeAgentDid) {
    return NextResponse.json(
      { error: { code: 'FORBIDDEN', message: 'You are not the consumer for this negotiation' } },
      { status: 403 }
    )
  }

  const transition = applyConsumerDeliveryReplyTransition(negotiation)
  if (!transition.ok) {
    return NextResponse.json(
